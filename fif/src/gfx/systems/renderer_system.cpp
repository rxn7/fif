...
namespace fif::gfx {
	void renderer_system([[maybe_unused]] const core::ApplicationStatus &status, entt::registry &registry) {
		...
		registry.view<CircleComponent, core::TransformComponent>().each([&]([[maybe_unused]] core::EntityID entity, CircleComponent &circle, core::TransformComponent &trans) {
			renderer.add_render_command<CircleRenderCommand>(circle.zIndex, static_cast<u32>(entity), trans.position, circle.radius, circle.tint);
		});
		...
		registry.view<QuadComponent, core::TransformComponent>().each([&]([[maybe_unused]] core::EntityID entity, QuadComponent &quad, core::TransformComponent &trans) {
			renderer.add_render_command<QuadRenderCommand>(quad.zIndex, static_cast<u32>(entity), trans.position, trans.scale * quad.size, trans.angleRadians, quad.tint);
		});
		...
		registry.view<SpriteComponent, core::TransformComponent>().each([&]([[maybe_unused]] core::EntityID entity, SpriteComponent &sprite, core::TransformComponent &trans) {
			if(!sprite.p_Texture) {
				renderer.add_render_command<QuadRenderCommand>(sprite.zIndex, static_cast<u32>(entity), trans.position, trans.scale * sprite.size, trans.angleRadians, sprite.tint);
				return;
			}

			renderer.add_render_command<SpriteRenderCommand>(sprite.zIndex, static_cast<u32>(entity), trans.position, trans.scale * sprite.size, trans.angleRadians, sprite.tint, sprite.p_Texture);
		});
		...
		registry.view<LabelComponent, core::TransformComponent>().each([&]([[maybe_unused]] core::EntityID entity, LabelComponent &label, core::TransformComponent &trans) {
			renderer.add_render_command<TextRenderCommand>(label.zIndex, static_cast<u32>(entity), label.text, label.p_font ? *label.p_font : Font::get_default(), trans.position, trans.scale * label.fontSize,
														   trans.angleRadians, label.color, label.verticalAlign, label.horizontalAlign);
		});
	}
}// namespace fif::gfx
