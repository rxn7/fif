...
namespace fif::gfx {
	...
	void Renderer2D::renderQuad(const glm::vec2 &position, const glm::vec2 &size, const glm::vec4 &color) {
		...
		glm::mat4 modelMatrix = core::Transform::getModelMatrix(glm::vec3(position, 0.0f), glm::vec3(size, 1.0f) * 0.5f, glm::vec3(0.0f));
	...
	void Renderer2D::renderCircle(const glm::vec2 &position, float radius, const glm::vec4 &color) {
		...
		glm::mat4 modelMatrix = core::Transform::getModelMatrix(glm::vec3(position, 0.0f), glm::vec3(radius, radius, radius) * 0.5f, glm::vec3(0.0f));
