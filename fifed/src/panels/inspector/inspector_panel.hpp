...
namespace fifed {
	...
	public:
		...
		template<typename T, typename... Args> void draw_add_component_entry(const std::string &name, std::function<void(T &comp)> callback = nullptr, Args &&...args) {
			if(m_SelectedEntity.has_component<T>())
				return;

			if(ImGui::MenuItem(name.c_str())) {
				T &comp = m_SelectedEntity.add_component<T>(std::forward<Args>(args)...);
				if(callback != nullptr)
					callback(comp);
		...
		template<typename T> void draw_component(const std::string &name, std::function<void(T &)> drawFunc) {
			T *comp = m_SelectedEntity.try_get_component<T>();
			if(!comp)
				return;

			ImGui::PushID(typeid(T).hash_code() + static_cast<u32>(m_SelectedEntity.m_ID));
			...
			if(removeComponent)
				m_SelectedEntity.remove_component<T>();
