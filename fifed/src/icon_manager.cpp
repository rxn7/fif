...
namespace fifed {
	...
	bool IconManager::imgui_button(const std::string_view id_str, IconType type, const vec2 &size) {
		...
		return ImGui::ImageButton(id_str.data(), reinterpret_cast<ImTextureID>(m_Texture.get_id()), ImVec2(size.x, size.y), ImVec2(uv0.x, uv0.y), ImVec2(uv1.x, uv1.y));
	}
}// namespace fifed
